...
Cloudflow offers a novel solution to approach the creation, deployment, and management of streaming applications on Kubernetes by offering developers a toolbox that facilitate the application creation and a set of Kubernetes extensions that makes Cloudflow streaming applications native to Kubernetes.  
